...
function normalizeModules(
  ...
  for (const [routePrefix, moduleOptions] of Object.entries(modules)) {
    ...
    const moduleName = typeof moduleOptions.module === 'string'
      ? normalizeModuleName(moduleOptions.module)
      : undefined

    normalized[normalizedPrefix] = {
      module: moduleName || undefined,
      layout: typeof moduleOptions.layout === 'string' && moduleOptions.layout.length > 0
...
export function resolveLayoutOptions(
  ...
  const globalFallbackLayout = userOptions.globalFallbackLayout
